...
export default function CalendarCard({ className, ...props }: React.ComponentPropsWithRef<typeof Card>) {
    ...
    return (
        <Card id="calendar/reminders-card"
            className={cn('flex overflow-hidden w-full h-[28.125rem]', className)}
            {...props}
        >
            <AnimatePresence initial={false}>
                <CalendarView layout key='calendar' open={open} view={view} setView={setView} />
                <RemindersView layout key='reminders' open={open} view={view} setView={setView} />
            </AnimatePresence>
        </Card>
    );
}