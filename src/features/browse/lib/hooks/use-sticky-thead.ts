...
export function useStickyTHead(offsetElementId: string) {
    ...
    useEffect(() => {
        ...
        const handleScrollTop = debounce(() => {
            ...
            const headerRect = document.getElementById(offsetElementId)?.getBoundingClientRect();
            ...
            if (currentRect && headerRect) {
                ...
                animate('thead', {
                    y: scrollTop,
                    boxShadow: scrollTop > 0 ? '0 0 0.375rem rgba(0, 0, 0, 0.15)' : '0 0 0.375rem rgba(0, 0, 0, 0)',
