...
const Table = React.forwardRef<
  ...
  HTMLMotionProps<'table'> &
    React.HTMLAttributes<HTMLTableElement> & { motion?: boolean }
...
const TableHeader = React.forwardRef<
  ...
  HTMLMotionProps<'thead'> &
    React.HTMLAttributes<HTMLTableSectionElement> & { motion?: boolean }
>(({ className, motion = false, ...props }, ref) => {
...
const TableCell = React.forwardRef<
  ...
  HTMLMotionProps<'td'> &
    React.TdHTMLAttributes<HTMLTableCellElement> & { motion?: boolean }
>(({ className, motion = false, ...props }, ref) => {
...
const TableCaption = React.forwardRef<
  ...
  HTMLMotionProps<'caption'> &
    React.HTMLAttributes<HTMLTableCaptionElement> & { motion?: boolean }
>(({ className, motion = false, ...props }, ref) => {
...
const TableRow = React.forwardRef<
  HTMLTableRowElement,
  HTMLMotionProps<'tr'> &
    React.HTMLAttributes<HTMLTableRowElement> & { motion?: boolean }
>(({ className, motion = false, ...props }, ref) => {
  const defaultClassName =
    'border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted [&_td:last-child]:border-r-0 [&_th:last-child]:border-r-0';
...
const TableHead = React.forwardRef<
  HTMLTableCellElement,
  HTMLMotionProps<'th'> &
    React.ThHTMLAttributes<HTMLTableCellElement> & { motion?: boolean }
>(({ className, motion = false, ...props }, ref) => {
  const defaultClassName =
    'h-10 px-2 border-r text-left align-middle font-semibold text-sm text-foreground';
  return motion ? (
